...
class LibHyperScan(object):
    def __init__(self, libpath="/lib/libhs.so"):
    ...
    def scan_nostream(self, data, callback=None):
        if callback is None:
...
def hs_scan_evnet(_id, _from, _to, flags, context):
        print _id
        return 0
def main():
    lib = LibHyperScan()
    print lib.version()
    patternlist = ["Phpmyadmin", "fengyu", "asdfg"]
    idslist = [1,2,3]
    lib.compile_multi(patternlist, idslist)
    lib.scan_nostream("qwrrrrrwe/Phpmyadminjieshiefengyusdfaddasdfgasdfasd", hs_scan_evnet)
